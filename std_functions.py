        ...
        for line in map(str.strip, file):
            if line.startswith('model'):
                ...
                if _Model.startswith('BTC284N'):
                    ...
                    _Model = 'HHEX'
                elif _Model.startswith('BTC284C'):
                    _Model = 'BTC284C'
                break
    return _Model
...
def getLaserwavelengthfromEEPROM(parafilename):
    ...
    with open(parafilename) as file:
        for line in map(str.strip, file):
            if line.startswith('laser_wavelength') and not line.startswith('laser_wavelength_'):
...
def interpolation(xarray, yarray, x_interp):
    if len(xarray) != len(yarray):
        return -1
    else:
        tck = interpolate.splrep(xarray, yarray, s=0)
        y_interp = interpolate.splev(x_interp, tck, der=0)
        return y_interp


# function to convert pixel array to wavelength array
def convertP2WL(xarray, acoeff):
    length = len(xarray)
    wl_array = []
    for pix in range(length):
        tt = acoeff[0] + acoeff[1]*pix + acoeff[2]*pix*pix + acoeff[3]*pix*pix*pix
        wl_array.append(tt)
    return wl_array


def convertWL2RS(wlarray, laserwl):
    length = len(wlarray)
    RS_array = []
    for wl in range(length):
        rs = 1e7 * (1/laserwl - 1/wlarray[wl])
        RS_array.append(rs)
    return RS_array


# return first index of array element whose value is Value
